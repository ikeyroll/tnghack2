...
export default function AuthModal() {
  const { screen, setScreen, recipient, amount, authenticate, isAppFrozen, setAppFrozen } = useApp();
  const [mode, setMode] = useState<Mode>("choose");
  const [pin, setPin] = useState("");

  const isAppAuth = isAppFrozen || (screen === "auth" && !recipient && !amount);
    ...
    <>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-bold text-gray-900">{isAppAuth ? "Authenticate to Continue" : "Verify to pay"}</h3>
        </div>
        {!isAppAuth && (
      ...
      <div className="mt-2 text-sm text-gray-600">
        {isAppAuth ? (
          "Verify your identity to unlock the app and continue."
        ) : (
          <>
    ...
    <AnimatePresence>
      {screen === "auth" && (
        <>
          {isAppAuth ? (
            <motion.div
              className="absolute inset-0 bg-white z-[70] flex items-center justify-center p-5"
